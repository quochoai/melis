<?php
  ...
  $first_btn = true;
  $last_btn = true;
  $perPage = $def['perpage_service_relate'];
  $start = $page * $perPage;
  $mod2 = $_REQUEST['mod2'];
  $tableService = "services";
  $wh = $_REQUEST['wh'];
  $allServices = $h->getAll("id, name_vi, name_en", $tableService, $wh, "sort desc, id desc", "limit $start, $perPage");
  $msg = "";
  foreach ($allServices as $k => $service) {
    $name = $service["name_$lng"];
    $linkService = $lng.'/'.$def['link_fservice'].'/'.$mod2.'/'.chuoilink($service['name_vi']).'.html';
    $msg .=  '<div class="eachnewsrelate"><a href="'.$linkService.'" title="'.$name.'"><i class="fas fa-caret-right"></i> '.$name.'</a></div>';
  ...
  $count = $h->checkExist($tableService, $wh);
