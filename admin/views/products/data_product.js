jQuery(document).ready(function($) {
	...
	$(document).on('click', '.delete_product', function() {
		...
		if (confirm(conf)) {
			...
			$.post(link_delete, { id: id, table: table_product }, function(data) {
				if (data == '1')
					$('tr#product' + id).hide();
				else {
					if (data == '5') {
	...
	$(document).on('click', '.active_product', function() {
		...
		$.post(link_active_product, { id: id, acti: activ, table: table_product }, function(html) {
			if (html == '1') {
	...
	$(document).on('click', '.sort_product', function() {
		var id = $(this).attr('id');
		var sapxep = $(this).val();
		$.post(link_update_sort, { id: id, sapxep: sapxep, table: table_product }, function(data) {
			if (data == '5') {
				toastr.error(session_timeout);
				setTimeout(function() {
					window.location.reload();
	...
	$(document).on('click', '.reload_product', function() {
		$(table_id_product).DataTable().destroy();
		//$(table_id_product).DataTable().state.clear();
		fill_datatable();
	});
	...
	$(document).on('click', '.add_product', function() {
		...
		$.post(link_add_product, { product_id: product_id }, function(data) {
	...
	$(document).on('click', '#add_product', function() {
		let eNameProductVi = $('#name_vi_product');
		let eNameProductEn = $('#name_en_product');
		let name_vi_product = $.trim(eNameProductVi.val());
		let name_en_product = $.trim(eNameProductEn.val());
		if (name_vi_product == '') {
			toastr.error(not_name_product_vi);
			eNameProductVi.focus();
			eNameProductVi.addClass("is-invalid");
			return false;
		} else {
			eNameProductVi.removeClass("is-invalid");
			eNameProductVi.addClass("is-valid");
		}
		if (name_en_product == '') {
			toastr.error(not_name_product_en);
			eNameProductEn.focus();
			eNameProductEn.addClass("is-invalid");
			return false;
		} else {
			eNameProductEn.removeClass("is-invalid");
			eNameProductEn.addClass("is-valid");
		}
	...
	$(document).on('click', '.update_product', function() {
		...
		$.post(link_update_product, { id: id }, function(html) {
	...
	$(document).on('click', '#update_product', function() {
		let eNameProductVi = $('#name_vi_product_e');
		let eNameProductEn = $('#name_en_product_e');
		var name_vi_product = $.trim(eNameProductVi.val());
		var name_en_product = $.trim(eNameProductEn.val());
		if (name_vi_product == '') {
			toastr.error(not_name_product_vi);
			eNameProductVi.focus();
			eNameProductVi.addClass("is-invalid");
