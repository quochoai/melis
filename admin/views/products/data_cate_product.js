jQuery(document).ready(function($) {
	...
	function fill_datatable() {
		$(table_id_cate_product).DataTable({
	...
	$(document).on('click', '.active', function() {
		...
		$.post(link_active, { id: id, acti: activ, table: table_cate_product }, function(html) {
			if (html == '1') {
	...
	$(document).on('click', '.reload', function() {
		$(table_id_cate_product).DataTable().destroy();
		//$(table_id_cate_product).DataTable().state.clear();
		fill_datatable();
	});
	...
	$(document).on('change', '.sort', function() {
		var id = $(this).attr('id');
		var sapxep = $(this).val();
		$.post(link_update_sort, { id: id, sapxep: sapxep, table: table_cate_product }, function(data) {
			if (data == '5') {
				toastr.error(session_timeout);
				setTimeout(function() {
					window.location.reload();
	...
	$(document).on('click', '#add', function() {
		var eNameVi = $('#name_vi_cate_product');
		var eNameEn = $('#name_en_cate_product');
		let name_vi = $.trim(eNameVi.val());
		let name_en = $.trim(eNameEn.val());
		if (name_vi == '') {
			toastr.error(not_name_category_product_vi);
			eNameVi.focus();
			eNameEn.addClass("is-invalid");
			return false;
		} else {
			eNameVi.removeClass("is-invalid");
			eNameVi.addClass("is-valid");
		}
		if (name_en == '') {
			...
			eNameEn.removeClass("is-invalid");
			eNameEn.addClass("is-valid");
		}
		$('#form_add').ajaxForm({
			beforeSend: function() {
	...
	$(document).on('click', '#update', function() {
		let eNameVi = $('#name_vi_cate_product_e');
		let eNameEn = $('#name_en_cate_product_e');
		let name_vi = $.trim(eNameVi.val());
		let name_en = $.trim(eNameEn.val());
		if (name_vi == '') {
			toastr.error(not_name_category_product_vi);
			eNameVi.focus();
			eNameVi.addClass("is-invalid");
			return false;
		} else {
			eNameVi.removeClass("is-invalid");
			eNameVi.addClass("is-valid");
