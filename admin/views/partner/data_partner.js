jQuery(document).ready(function($) {
	...
	$(document).on('click', '.delete_partner', function() {
		...
		if (confirm(conf)) {
			...
			$.post(link_delete, { id: id, table: table_partner }, function(data) {
				if (data == '1')
					$('tr#partner' + id).hide();
				else {
					if (data == '5') {
	...
	$(document).on('click', '.active_partner', function() {
		...
		$.post(link_active, { id: id, acti: activ, table: table_partner }, function(html) {
			if (html == '1') {
	...
	$(document).on('change', '.sort_partner', function() {
		var id = $(this).attr('id');
		var sapxep = $(this).val();
		$.post(link_update_sort, { id: id, sapxep: sapxep, table: table_partner }, function(data) {
			if (data == '5') {
				toastr.error(session_timeout);
				setTimeout(function() {
					window.location.reload();
	...
	$(document).on('click', '.reload_partner', function() {
		$(table_id_partner).DataTable().destroy();
		fill_datatable();
	});
	...
	$(document).on('click', '.add_partner', function() {
		...
		$.post(link_add_partner, { ld_id: ld_id }, function(data) {
	...
	$(document).on('click', '#add_partner', function() {
		let eNamePartnerVi = $('#name_vi_partner');
		let eNamePartnerEn = $('#name_en_partner');
		let name_vi = $.trim(eNamePartnerVi.val());
		let name_en = $.trim(eNamePartnerEn.val());
		let image_partner = $.trim($('#image_partner').val());
		if (name_vi == '') {
			toastr.error(not_enter_name_partner_vi);
			eNamePartnerVi.focus();
			eNamePartnerVi.addClass("is-invalid");
			return false;
		} else {
			eNamePartnerVi.removeClass("is-invalid");
			eNamePartnerVi.addClass("is-valid");
		}
		if (name_en == '') {
			toastr.error(not_enter_name_partner_en);
			eNamePartnerEn.focus();
			eNamePartnerEn.addClass("is-invalid");
			return false;
		} else {
			eNamePartnerEn.removeClass("is-invalid");
			eNamePartnerEn.addClass("is-valid");
		}
		if (image_partner == '') {
	...
	$(document).on('click', '.update_partner', function() {
		...
		$.post(link_update_partner, { id: id }, function(html) {
			$('#partner_update').html(html);
			$('#modal-update-partner').modal('show');
		});
	});
	$(document).on('click', '#update_partner', function() {
		let eNamePartnerVi = $('#name_vi_partner_e');
		let eNamePartnerEn = $('#name_en_partner_e');
		let name_vi = $.trim(eNamePartnerVi.val());
		let name_en = $.trim(eNamePartnerEn.val());
		if (name_vi == '') {
