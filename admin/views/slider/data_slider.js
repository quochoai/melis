jQuery(document).ready(function($) {
	...
	$(document).on('click', '.delete_slider', function() {
		...
		if (confirm(conf)) {
			...
			$.post(link_delete, { id: id, table: table_slider }, function(data) {
				if (data == '1')
					$('tr#slider' + id).hide();
				else {
					if (data == '5') {
	...
	$(document).on('click', '.active_slider', function() {
		...
		$.post(link_active, { id: id, acti: activ, table: table_slider }, function(html) {
			if (html == '1') {
	...
	$(document).on('change', '.sort_slider', function() {
		var id = $(this).attr('id');
		var sapxep = $(this).val();
		$.post(link_update_sort, { id: id, sapxep: sapxep, table: table_slider }, function(data) {
			if (data == '5') {
				toastr.error(session_timeout);
				setTimeout(function() {
					window.location.reload();
	...
	$(document).on('click', '.reload_slider', function() {
		$(table_id_slider).DataTable().destroy();
		fill_datatable();
	});
	...
	$(document).on('click', '.add_slider', function() {
		...
		$.post(link_add_slider, { hc_id: hc_id }, function(data) {
	...
	$(document).on('click', '.update_slider', function() {
		...
		$.post(link_update_slider, { id: id }, function(html) {
