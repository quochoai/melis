jQuery(document).ready(function($) {
	...
	$(document).on('click', '.delete_price_service', function() {
		...
		if (confirm(conf)) {
			...
			$.post(link_delete, { id: id, table: table_price }, function(data) {
				if (data == '1')
	...
	$(document).on('click', '.active_price_service', function() {
		...
		$.post(link_active, { id: id, acti: activ, table: table_price }, function(html) {
			if (html == '1') {
	...
	$(document).on('change', '.sort_price_service', function() {
		var id = $(this).attr('id');
		var sapxep = $(this).val();
		$.post(link_update_sort, { id: id, sapxep: sapxep, table: table_price }, function(data) {
			if (data == '5') {
				toastr.error(session_timeout);
				setTimeout(function() {
					window.location.reload();
	...
	$(document).on('click', '.reload_price_service', function() {
		$(table_id_price).DataTable().destroy();
		//$(table_id_price).DataTable().state.clear();
		fill_datatable();
	});
	...
	$(document).on('click', '.add_price_service', function() {
		...
		$.post(link_add_price, { service_id: service_id }, function(data) {
	...
	$(document).on('click', '#add_price_service', function() {
		let eNamePriceVi = $('#name_vi_price');
		let eNamePriceEn = $('#name_en_price');
		let name_vi = $.trim(eNamePriceVi.val());
		let name_en = $.trim(eNamePriceEn.val());
		if (name_vi == '') {
			toastr.error(not_name_price_service_vi);
			eNamePriceVi.focus();
			eNamePriceVi.addClass("is-invalid");
			return false;
		} else {
			eNamePriceVi.removeClass("is-invalid");
			eNamePriceVi.addClass("is-valid");
		}
		if (name_en == '') {
			toastr.error(not_name_price_service_en);
			eNamePriceEn.focus();
			eNamePriceEn.addClass("is-invalid");
			return false;
		} else {
			eNamePriceEn.removeClass("is-invalid");
			eNamePriceEn.addClass("is-valid");
		}
	...
	$(document).on('click', '#update_price_service', function() {
		let eNamePriceVi = $('#name_vi_price_e');
		let eNamePriceEn = $('#name_en_price_e');
		var name_vi = $.trim(eNamePriceVi.val());
		var name_en = $.trim(eNamePriceEn.val());
		if (name_vi == '') {
			toastr.error(not_name_price_service_vi);
			eNamePriceVi.focus();
			eNamePriceVi.addClass("is-invalid");
